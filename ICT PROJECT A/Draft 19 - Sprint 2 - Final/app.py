...
def mfa_validate():
    print("=== MFA VALIDATION DEBUG START ===")
    ...
    print(f"OTP length: {len(otp) if otp else 0}")

    if not user_id or not otp:
        print("ERROR: Missing user ID or OTP")
    ...
    try:
        # Get user data
        user_result = supabase.table("users").select("email, role, mfa_secret").eq("id", user_id).single().execute()
        print(f"User query result: {user_result.data}")
        
        if not user_result.data:
            print("ERROR: User not found")
            return render_template("mfa.html", error="User not found", user_id=user_id)
            
        user_info = user_result.data
        mfa_secret = user_info.get("mfa_secret")
        
        if not mfa_secret:
            print("ERROR: No MFA secret found for user")
            return render_template("mfa.html", error="MFA not set up", user_id=user_id)
            
        print(f"MFA secret exists: {bool(mfa_secret)}")
        ...
        print(f"User entered: {otp}")
        
        # Verify OTP with larger time window
        is_valid = totp.verify(otp, valid_window=5)
        print(f"OTP validation result: {is_valid}")
        
        if not is_valid:
            print("MFA verification FAILED")
            log_activity(user_id, f"MFA verification failed: Invalid OTP", email=user_info.get("email"))
            return render_template("mfa.html", error="Invalid OTP code. Please try again.", user_id=user_id)
        
        print("MFA verification SUCCESSFUL")
        log_activity(user_id, "MFA verification successful", email=user_info.get("email"))

        # Generate JWT token
        print("Generating JWT token...")
        new_token = generate_jwt(user_id, user_info["role"], mfa_verified=True)
        print(f"JWT generated: {bool(new_token)}")
        
        log_activity(user_id, "JWT generated after MFA verification", email=user_info.get("email"))
        ...
        print(f"EXCEPTION in MFA validation: {str(e)}")
        print(f"Exception type: {type(e)}")
        import traceback
        traceback.print_exc()
        
        try:
            email = user_info.get("email") if 'user_info' in locals() else None
        except Exception:
            email = None
            
        log_activity(user_id, f"MFA verification failed: {str(e)}", email=email)
        return render_template("mfa.html", error=f"Verification failed: {str(e)}", user_id=user_id)
    
    finally:
        print("=== MFA VALIDATION DEBUG END ===")
        print("")
...
def reset_password(token):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            try:
                # ONLY update Supabase Auth - remove local hashing
